...
def is_lite_gemini_model(model_name):
    return "lite" in normalize_gemini_model_name(model_name).lower()

def preferred_economy_gemini_model(models):
    models = [normalize_gemini_model_name(model) for model in (models or [])]
    for preferred in GEMINI_FALLBACK_MODELS:
        if preferred in models and is_lite_gemini_model(preferred):
            return preferred
    for model in models:
        if is_lite_gemini_model(model):
            return model
    return preferred_gemini_model(models)
...
def render_api_sidebar(gemini_default, mp_default, semantic_scholar_default, lang_code):
    ...
    with st.sidebar:
        ...
        economy_mode = st.checkbox(
            ...
            help="يفضل موديلات Lite ويقلل احتمال نفاد حصة Gemini المجانية.",
        )
        preferred_model = preferred_economy_gemini_model(gemini_model_options) if economy_mode else preferred_gemini_model(gemini_model_options)
        if (
            "gemini_model_input" not in st.session_state
            or st.session_state.gemini_model_input not in gemini_model_options
            or (economy_mode and not is_lite_gemini_model(st.session_state.gemini_model_input))
        ):
            st.session_state.gemini_model_input = preferred_model
        ...
        if economy_mode and not is_lite_gemini_model(gemini_model):
            st.warning("الوضع الاقتصادي مفعّل. اختر موديل Lite لتجنب خطأ 429.")
...
def build_transcript_text(messages):
    lines = []
    for index, msg in enumerate(_visible_discussion_messages(messages), 1):
        name = _agent_display_name(_message_name(msg))
        content = _message_content(msg)
        lines.append(f"[{index}] {name}")
        lines.append(content if content else "(no text content)")
        if msg.get("function_call"):
            lines.append(json.dumps(msg["function_call"], ensure_ascii=False, indent=2))
        lines.append("")
    return "\n".join(lines).strip() or "No agent messages were produced yet."

def extract_material_snapshot(messages):
    text = "\n".join(_message_content(msg) for msg in _visible_discussion_messages(messages))
    ids = re.findall(r"mp-\d+", text)
    material_id = ids[-1] if ids else None
    bandgap = None
    bandgap_patterns = [
        r"(?:band\s*gap|bandgap|Eg|E_g)[^\d]{0,40}(\d+(?:\.\d+)?)",
        r"(\d+(?:\.\d+)?)\s*eV",
    ]
    for pattern in bandgap_patterns:
        matches = re.findall(pattern, text, flags=re.IGNORECASE)
        numeric = [float(value) for value in matches if 0.3 <= float(value) <= 4.0]
        if numeric:
            bandgap = numeric[-1]
            break
    formulas = re.findall(r"\b(?:[A-Z][a-z]?\d*){2,}\b", text)
    formula = formulas[-1] if formulas else None
    return material_id, formula, bandgap

def render_downloads_section(messages, task_theme, error_text=None):
    st.header(T("downloads"))
    visible_messages = _visible_discussion_messages(messages)
    if not visible_messages:
        st.info("لا توجد رسائل وكلاء بعد. عند صدور أول رد ستظهر هنا ملفات المحضر والنتائج.")
    transcript = build_transcript_text(messages)
    st.download_button(
        "تنزيل محضر الحوار (TXT)",
        transcript,
        "agent_discussion.txt",
        mime="text/plain",
        key=f"download_transcript_{len(messages)}_{bool(error_text)}",
    )
    st.download_button(
        "تنزيل بيانات الجلسة (JSON)",
        json.dumps(visible_messages, ensure_ascii=False, indent=2, default=str),
        "agent_messages.json",
        mime="application/json",
        key=f"download_messages_{len(messages)}_{bool(error_text)}",
    )
    if error_text:
        st.warning("تم إنشاء ملفات المحضر والرسائل المتاحة قبل توقف التنفيذ.")
        st.download_button(
            "تنزيل تقرير الخطأ (TXT)",
            str(error_text),
            "mission_error.txt",
            mime="text/plain",
            key=f"download_error_{len(messages)}",
        )
        return

    material_id, formula, bandgap = extract_material_snapshot(messages)
    if material_id and (not formula or bandgap is None):
        dft_res = dft_analysis(material_id)
        data = dft_res.get("json_data") or {}
        formula = formula or data.get("formula")
        bandgap = bandgap if bandgap is not None else data.get("band_gap_GGA")
        if data:
            st.download_button(
                T("dft_json"),
                json.dumps(data, indent=2),
                f"dft_{material_id}.json",
                mime="application/json",
            )
    elif material_id:
        dft_res = dft_analysis(material_id)
        if dft_res.get("json_data"):
            st.download_button(
                T("dft_json"),
                json.dumps(dft_res["json_data"], indent=2),
                f"dft_{material_id}.json",
                mime="application/json",
            )

    if not (formula and bandgap is not None):
        st.info("لم أجد مادة كاملة مع فجوة طاقة داخل الحوار بعد. شغّل أدوارًا أكثر أو تأكد أن وكيل المواد يستخدم الأداة، وستظهر ملفات SCAPS وPDF والملصق هنا.")
        return

    scaps_res = scaps_simulation(formula, float(bandgap))
    if scaps_res.get("scaps_file"):
        st.download_button(T("scaps_file"), scaps_res["scaps_file"], f"scaps_{formula}.scaps", mime="text/plain")

    thicknesses = np.linspace(100, 1000, 10)
    pces = [scaps_simulation(formula, float(bandgap), thickness=th / 1000)["pce"] for th in thicknesses]
    fig, ax = plt.subplots()
    ...
    pdf.cell(200, 10, text=f"Report: {formula}", ln=True)
    pdf.cell(200, 10, text=f"Task: {task_theme}", ln=True)
    pdf.cell(200, 10, text=f"Band gap: {fmt_number(bandgap, 2)} eV", ln=True)
    pdf.cell(200, 10, text=f"Estimated PCE: {fmt_number(scaps_res.get('pce'), 2)}%", ln=True)
    pdf_data = pdf.output(dest="S")
    if isinstance(pdf_data, str):
        pdf_data = pdf_data.encode("latin-1", errors="ignore")
    st.download_button(T("pdf_report"), pdf_data, "report.pdf", mime="application/pdf")

    img = Image.new("RGB", (800, 400), "white")
    d = ImageDraw.Draw(img)
    d.text((50, 50), f"Material: {formula}", fill="black")
    d.text((50, 100), f"Band gap: {fmt_number(bandgap, 2)} eV", fill="black")
    d.text((50, 150), f"Efficiency: {fmt_number(scaps_res.get('pce'), 1)}%", fill="black")
    poster_buf = io.BytesIO()
    img.save(poster_buf, format="PNG")
    poster_buf.seek(0)
    st.download_button(T("poster"), poster_buf.getvalue(), "poster.png", mime="image/png")
...
def is_gemini_quota_error(error_text):
    text = str(error_text or "")
    return "429" in text or "TooManyRequests" in text or "quota" in text.lower()
...
IP rights: {ip_rights}
    ...
    if mission_state["error"]:
        if is_gemini_quota_error(mission_state["error"]):
            fallback_model = preferred_economy_gemini_model(
                st.session_state.get("available_gemini_models") or GEMINI_FALLBACK_MODELS
            )
            st.session_state.gemini_economy_mode = True
            st.session_state.gemini_model_input = fallback_model
            st.info(f"تم تحويل اختيار Gemini تلقائيًا إلى {fallback_model}. أعد تشغيل المهمة بعد انتهاء مهلة الحصة.")
        with st.expander("التفاصيل التقنية", expanded=False):
            st.code(mission_state["traceback"], language="python")
        render_downloads_section(messages, task_theme, error_text=mission_state["traceback"])
        st.stop()

    st.success(f"تم إنجاز المهمة في {duration:.0f} ثانية")
    render_downloads_section(messages, task_theme)
