...
def render_downloads_section(messages, task_theme, error_text=None):
    ...
    pdf.cell(200, 10, txt=f"Report: {formula}", ln=True)
    pdf.cell(200, 10, txt=f"Task: {task_theme}", ln=True)
    pdf.cell(200, 10, txt=f"Band gap: {fmt_number(bandgap, 2)} eV", ln=True)
    pdf.cell(200, 10, txt=f"Estimated PCE: {fmt_number(scaps_res.get('pce'), 2)}%", ln=True)
    ...
    d.txt((50, 50), f"Material: {formula}", fill="black")
    d.txt((50, 100), f"Band gap: {fmt_number(bandgap, 2)} eV", fill="black")
    d.txt((50, 150), f"Efficiency: {fmt_number(scaps_res.get('pce'), 1)}%", fill="black")
