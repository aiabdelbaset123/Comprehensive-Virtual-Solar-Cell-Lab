...
import os, time, random, json, tempfile, textwrap, re, io, base64, copy
...
try:
    ...
    st.error("❌ مفاتيح API غير مضبوطة. أضفها في Secrets.")
    st.stop()
...
if st.session_state.get("start_mission", False):
    ...
    user_proxy = UserProxyAgent(name="human", human_input_mode="NEVER", llm_config=False)
...
IP rights: {ip_rights}
    ...
    with st.spinner("🧠 الوكلاء ينفذون المهمة..."):
        start = time.time()
        user_proxy.initiate_chat(manager, message=initial_message)
        messages = groupchat.messages
        duration = time.time()-start
    st.success(f"✅ Done in {duration:.0f} sec")
